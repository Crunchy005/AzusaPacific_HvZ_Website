<?php
	...
	if(check_admin_status($_SESSION['username']))
	{
		...
		{	
			...
			$q = "select f_name, l_name, pid from Player where status = 3 order by f_name ASC";//get all human players
			...
			while($result_array = mysqli_fetch_assoc($result))
			{
				...
				if(mysqli_num_rows($result2) == 0 && $mission != 0)
				{
					...
					"</li><Br>";//output the players with checkbox inputs.
				}
			}
		}
		else if(isset($_GET['pid']) && isset($_GET['mission_id']))
		{
			...
			if($result)
				...
				$no_error = false;
			//echo("attendance");
			$q = "update Player set vaccinated = 1 where pid = '$pid'";
			$result = mysqli_query($con, $q) or die ("Error vaccinating player " . mysqli_error($con));
			//echo("vaccinated");
			if($result)
				$no_error = true;
			else
